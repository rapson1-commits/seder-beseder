  ...
  useEffect(() => {
    ...
    async function loadData() {
      const [familyRes, membersRes, eventsRes] = await Promise.all([
        ...
        supabase.from('family_members').select('*').eq('family_id', familyId).eq('is_active', true).order('first_name'),
        supabase.from('events').select('*').eq('family_id', familyId).order('created_at', { ascending: false }).limit(5),
      ])

      if (familyRes.data) setFamilyName(familyRes.data.family_name || 'המשפחה שלי')

      const m = (membersRes.data ?? []) as FamilyMember[]
      const e = (eventsRes.data ?? []) as Event[]
      setMembers(m)
      setEvents(e)

      // Simple side-balance insight (uses correct field: side_of_family)
      if (m.length > 0) {
        const dadSide = m.filter(x => x.side_of_family === 'אבא').length
        const momSide = m.filter(x => x.side_of_family === 'אמא').length
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'10px', marginBottom:'16px' }}>
          {members.slice(0, 4).map((m, i) => (
            <button key={m.id} onClick={() => router.push(`/members/${m.id}`)}
              ...
              <div style={{ width:'56px', height:'56px', borderRadius:'16px', background:avatarColor(i), display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', fontWeight:800, color:'white' }}>
                {m.first_name[0]}
              </div>
              <span style={{ fontSize:'11px', fontWeight:600, color:'rgba(255,255,255,.7)' }}>{m.first_name}</span>
            </button>
          ))}
          {members.length === 0 && (
            ...
            {events.slice(0, 3).map((ev, i) => (
              <button key={ev.id} onClick={() => router.push(`/events/${ev.id}`)}
                ...
                <div style={{ flex:1, textAlign:'right' }}>
                  ...
                  <div style={{ color:'rgba(255,255,255,.4)', fontSize:'12px', marginTop:'2px' }}>{ev.location_name ?? ''}</div>
                </div>
