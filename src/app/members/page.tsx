...
export default function MembersPage() {
  ...
  const COLORS = ['#4A82D4','#4E9B6A','#F07A55','#C99A2E','#E0655F','#9B59B6']
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const { data } = await supabase.from('family_members').select('*').eq('family_id', profile.family_id).order('first_name')
      setMembers(data || [])
      setLoading(false)
    })
  }, [router])
  ...
  const sideColor: Record<string,string> = { 'צד אבא':'#4A82D4', 'צד אמא':'#E0655F', 'בן/בת זוג':'#4E9B6A' }
    ...
    <div style={{ minHeight:'100vh', background:'#192542', fontFamily:'Heebo, sans-serif', direction:'rtl' }}>
      ...
      <div style={{ maxWidth:'430px', margin:'0 auto', padding:'16px 16px 100px' }}>
        ...
        <div style={{ background:'rgba(255,255,255,.06)', borderRadius:'16px', padding:'14px 20px', marginBottom:'16px', display:'flex', justifyContent:'space-around' }}>
          ...
          <div style={{ textAlign:'center' }}><div style={{ color:'white', fontSize:'22px', fontWeight:900 }}>{members.filter(m=>m.side_of_family==='צד אבא').length}</div><div style={{ color:'rgba(255,255,255,.4)', fontSize:'11px' }}>צד אבא</div></div>
          <div style={{ textAlign:'center' }}><div style={{ color:'white', fontSize:'22px', fontWeight:900 }}>{members.filter(m=>m.side_of_family==='צד אמא').length}</div><div style={{ color:'rgba(255,255,255,.4)', fontSize:'11px' }}>צד אמא</div></div>
        </div>
        ...
        {members.length === 0 ? (
          ...
          const sc = sideColor[m.side_of_family] || '#888'
          return (
            <div key={m.id} style={{ background:'rgba(255,255,255,.06)', borderRadius:'16px', padding:'14px 16px', marginBottom:'10px', display:'flex', alignItems:'center', gap:'12px', border:'1px solid rgba(255,255,255,.08)' }}>
              <div style={{ width:'48px', height:'48px', borderRadius:'14px', background:color, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px', fontWeight:800, color:'white', flexShrink:0 }}>{initials}</div>
              <div style={{ flex:1 }}>
      ...
      <nav style={{ position:'fixed', bottom:0, left:0, right:0, height:'72px', background:'white', borderTop:'1px solid #eee', display:'flex', zIndex:99, paddingBottom:'8px' }}>
        {[['🏠','בית','/home',false],['👥','משפחה','/members',true],['📅','היסטוריה','/history',false],['💡','תובנות','/insights',false],['⚙️','הגדרות','/settings',false]].map(([i,l,p,a]) => (
