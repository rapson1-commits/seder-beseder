    ...
    if (!isNew) {
      ...
      if (member) {
        setForm({
          ...
          phone: member.phone || '',
          family_side: member.family_side || '',
      ...
      <div style={{ background:'linear-gradient(135deg,#192542,#1e3a6e)', padding:'52px 20px 20px', position:'sticky', top:0, zIndex:10 }}>
        <div style={{ maxWidth:'430px', margin:'0 auto', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          <h1 style={{ color:'white', fontSize:'20px', fontWeight:800 }}>
            {isNew ? 'הוסף בן/בת משפחה' : 'עריכת פרטים'}
          </h1>
          <button
            onClick={save}
            disabled={saving}
            style={{ background:'#F07A55', border:'none', borderRadius:'12px', padding:'8px 14px', color:'white', fontSize:'13px', fontWeight:700, cursor:'pointer', opacity: saving ? 0.7 : 1, fontFamily:'Heebo, sans-serif' }}>
            {saving ? '...' : '✓ שמור'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'430px', margin:'0 auto', padding:'20px 16px 100px' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:'24px' }}>
          <div style={{ width:'72px', height:'72px', borderRadius:'22px', background:'#F07A55', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'26px', fontWeight:900, color:'white', margin:'0 auto' }}>
            {form.full_name ? form.full_name.split(' ').map(n=>n[0]).join('').slice(0,2) : '?'}
          </div>
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
          ...
          <div>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:'8px' }}>
              {sides.map(side => (
                <button
                  key={side}
                  onClick={() => setForm(p => ({...p, family_side: side}))}
                  style={{
                    padding: '8px 16px',
                    borderRadius: '100px',
                    border: '1.5px solid',
                    borderColor: form.family_side === side ? '#F07A55' : 'rgba(255,255,255,.15)',
                    background: form.family_side === side ? 'rgba(240,122,85,.2)' : 'transparent',
                    color: form.family_side === side ? '#F07A55' : 'rgba(255,255,255,.5)',
                    fontSize: '13px',
                    fontWeight: 700,
                    cursor: 'pointer',
                    fontFamily: 'Heebo, sans-serif',
          ...
          <div>
            ...
            <input
              style={inputStyle}
              placeholder="לדוגמה: דוד, אחות, בן זוג..."
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="אלרגיות, העדפות, הערות..."
              value={form.notes}
        ...
        {error && (
          <div style={{ background:'rgba(224,101,95,.15)', border:'1px solid rgba(224,101,95,.3)', borderRadius:'12px', padding:'12px 16px', marginTop:'16px', color:'#E0655F', fontSize:'13px', fontWeight:600 }}>
            ⚠️ {error}
          </div>
        )}
        ...
        <button
          onClick={save}
          disabled={saving}
          style={{ width:'100%', padding:'15px', background:'#F07A55', border:'none', borderRadius:'16px', color:'white', fontSize:'15px', fontWeight:700, cursor:'pointer', marginTop:'24px', boxShadow:'0 6px 20px rgba(240,122,85,.35)', opacity: saving ? 0.7 : 1, fontFamily:'Heebo, sans-serif' }}>
          {saving ? 'שומר...' : isNew ? '+ הוסף למשפחה' : '✓ שמור שינויים'}
        </button>
      </div>
    </div>
  )
